...
_compiled_train_fn = None


def inner_steps(model, data_iterator, optimizer, num_steps, device):
    global _is_initialized, _compiled_train_fn
    ...
    if is_cuda_device:
        try:
            for param_group in optimizer.param_groups:
                if not param_group.get("fused", False):
                    param_group["fused"] = True
                    param_group["foreach"] = False
        except Exception:
            pass

    if _compiled_train_fn is None:
        device_type = device.type

        def eager_train_step(input_ids, labels):
        ...
        if is_cuda_device:
            try:
                _compiled_train_fn = torch.compile(
                    eager_train_step,
                    mode="reduce-overhead",
                    fullgraph=False,
                )
            except Exception:
                _compiled_train_fn = eager_train_step
        else:
            _compiled_train_fn = eager_train_step
    ...
    if is_cuda_device:
        ...
        with torch.cuda.stream(prefetch_stream):
            next_batch_data = next(data_iterator).to(device, non_blocking=True)
    else:
        next_batch_data = next(data_iterator).to(device)

    total_tokens_count = 0
    final_loss_value = 0.0
    final_logits_tensor = None

    for step_idx in range(num_steps):
        if is_cuda_device:
            torch.cuda.current_stream().wait_stream(prefetch_stream)
        current_batch = next_batch_data

        if step_idx < num_steps - 1:
            upcoming_batch = next(data_iterator)
            if is_cuda_device:
                with torch.cuda.stream(prefetch_stream):
                    next_batch_data = upcoming_batch.to(device, non_blocking=True)
            else:
                next_batch_data = upcoming_batch.to(device)

        loss_val, logits_val = _compiled_train_fn(current_batch[:, :-1], current_batch[:, 1:])

        optimizer.step()
        optimizer.zero_grad(set_to_none=True)

        total_tokens_count += current_batch.numel()
        if step_idx == num_steps - 1:
            final_loss_value = loss_val.item()
            final_logits_tensor = logits_val.float()
