...
def inner_steps(model, data_iterator, optimizer, num_steps, device):
    ...
    if _compiled_step is None:
        ...
        def _raw_step(input_ids, labels):
            with torch.autocast(device_type=dev_kind, dtype=torch.bfloat16):
    ...
    if use_cuda:
        ...
        with torch.cuda.stream(prefetch_stream):
            upcoming_batch = next(data_iterator).to(device, non_blocking=True)
    else:
        upcoming_batch = next(data_iterator).to(device)
    ...
    for step_idx in range(num_steps):
        ...
        current_batch = upcoming_batch

        if step_idx < num_steps - 1:
            raw_batch = next(data_iterator)
            if use_cuda:
                with torch.cuda.stream(prefetch_stream):
                    upcoming_batch = raw_batch.to(device, non_blocking=True)
            else:
                upcoming_batch = raw_batch.to(device)
        ...
        total_tokens += current_batch.numel()
        if step_idx == num_steps - 1:
...
if __name__ == "__main__":
    ...
    print()
    tps_results.sort()
    median_tps = tps_results[len(tps_results) // 2]
